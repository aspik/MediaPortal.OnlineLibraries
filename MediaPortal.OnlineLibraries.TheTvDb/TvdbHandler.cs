...
using System.Linq;
using MediaPortal.OnlineLibraries.TheTvDb.Cache;
using MediaPortal.OnlineLibraries.TheTvDb.Data;
using MediaPortal.OnlineLibraries.TheTvDb.Data.Banner;
using MediaPortal.OnlineLibraries.TheTvDb.Data.Comparer;
using MediaPortal.OnlineLibraries.TheTvDb.Exceptions;
...
namespace MediaPortal.OnlineLibraries.TheTvDb
{
  ...
  public class TvdbHandler
  {
    ...
    #region Nested type: UpdateFinishedEventArgs
    ...
    #region Nested type: UpdateProgressEventArgs
    ...
    {
      get { return (_loadedData != null && _loadedData.LanguageList != null && _loadedData.LanguageList.Count > 0); }
    }
                                ...
                                bool loadActors, bool loadBanners, bool useZip)
    {
      ...
      if (series == null || //series not yet cached
          (useZip && (!series.EpisodesLoaded && !series.TvdbActorsLoaded && !series.BannersLoaded)))
        //only the basic series info has been loaded -> zip is still faster than fetching the missing informations without using zip
      {
    ...
    /// <exception cref="TvdbContentNotFoundException">The episode/series/banner couldn't be located on the tvdb server.</exception>
    /// <exception cref="TvdbNotAvailableException">Exception is thrown when thetvdb isn't available.</exception>
    public TvdbEpisode GetEpisode(int seriesId, int seasonNr, int episodeNr, TvdbEpisode.EpisodeOrdering order,
      ...
      if (_cacheProvider != null && _cacheProvider.Initialised)
      {
        if (_cacheProvider.IsCached(seriesId, language, true, false, false))
        {
          ...
          if (series.Episodes != null)
          {
            ...
            {
              if (e.EpisodeNumber == episodeNr && e.SeasonNumber == seasonNr &&
                  order == TvdbEpisode.EpisodeOrdering.DefaultOrder ||
                  e.DvdEpisodeNumber == episodeNr && e.SeasonNumber == seasonNr &&
                  order == TvdbEpisode.EpisodeOrdering.DvdOrder ||
    ...
    public TvdbEpisode GetEpisode(int seriesId, DateTime airDate, TvdbLanguage language)
    {
      ...
      if (_cacheProvider != null && _cacheProvider.Initialised)
      {
        if (_cacheProvider.IsCached(seriesId, language, true, false, false))
        {
          ...
          foreach (TvdbEpisode e in series.Episodes)
          {
            if (e.FirstAired.Year == airDate.Year && e.FirstAired.Month == airDate.Month &&
                e.FirstAired.Day == airDate.Day)
            {
                                  ...
                                  bool replaceArtwork)
    {
      ...
      TvdbSeries newSeries = _downloader.DownloadSeries(series.Id, series.Language, loadEpisodes, loadActors,
                                                        loadBanners);
    ...
    public List<int> AddSeriesToFavorites(int seriesId)
    {
      ...
      List<int> list = _downloader.DownloadUserFavoriteList(_userInfo.UserIdentifier, Util.UserFavouriteAction.Add,
                                                            seriesId);
    ...
    public List<int> RemoveSeriesFromFavorites(int seriesId)
    {
      if (_userInfo == null)
        throw new TvdbUserNotFoundException("You can only add favorites if a user is set");
      List<int> list = _downloader.DownloadUserFavoriteList(_userInfo.UserIdentifier, Util.UserFavouriteAction.Remove,
                                                            seriesId);
      _userInfo.UserFavorites = list;
      return list;
    }
    ...
    private bool MakeUpdate(Interval interval, bool zipped, bool reloadOldContent)
    {
      ...
      DateTime updateTime = _downloader.DownloadUpdate(out updateSeries, out updateEpisodes, out updateBanners, interval,
                                                       zipped);
      ...
      foreach (TvdbBanner b in updateBanners)
      {
        ...
        foreach (int s in cachedSeries)
        {
          if (b.SeriesId == s)
          {
            ...
            TvdbSeries series = seriesToSave.ContainsKey(s)
                                  ? seriesToSave[s]
                                  : _cacheProvider.LoadSeriesFromCache(b.SeriesId);
      ...
      if (UpdateFinished != null)
      {
        if (!_abortUpdate || _abortUpdateSaveChanges)
        {
          ...
          UpdateFinished(new UpdateFinishedEventArgs(startUpdate, DateTime.Now, updatedSeriesIds, updatedEpisodeIds,
                                                     updatedBannerIds));
        }
        else
        {
          //we didn't update anything because the update was aborted
          UpdateFinished(new UpdateFinishedEventArgs(startUpdate, DateTime.Now, new List<int>(), new List<int>(),
                                                     new List<int>()));
        }
      }
      return true;
    ...
    private bool UpdateEpisode(TvdbSeries series, TvdbEpisode episode, int progress, out String text)
    {
      ...
      foreach (KeyValuePair<TvdbLanguage, TvdbSeriesFields> kvp in series.SeriesTranslations)
      {
            ...
            foreach (TvdbEpisode e in eps.Where(e => e.Id == episode.Id))
            {
              ...
              if (e.LastUpdated < episode.LastUpdated)
              {
                ...
                if (newEpisode != null)
                {
                  ...
                  Log.Info("Updated Episode " + e.SeasonNumber + "x" + e.EpisodeNumber + " for series " +
                           series.SeriesName +
        ...
        {
          Log.Debug("Not handling episode " + episode.Id + ", because series " + series.SeriesName +
                    " hasn't loaded episodes");
          updateDone = false;
        }
      }
